...
function example4(n)
    ...
    ## error
    err = norm(unum - ubig , 2);  ## solution(Float64) - solution(exact)
...
for i = 1 : 1 : lx[1]
    global err[i] = example4(x[i]);
end

p1 = plot(x, log10.(err), color=:black, linewidth=2, xlabel=L"n", ylabel="error", label=:none, alpha=1, grid=false)
# p1 = plot(x, err, color=:black, linewidth=2, xlabel=L"n", ylabel="error", label=:none, alpha=1, grid=false, yaxis=:log)

# open("Fig1c.txt", "w") do file
    # for i in 1:length(x)
