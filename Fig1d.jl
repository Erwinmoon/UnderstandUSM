...
for i = 1 : 1 : lx[1]
    u1 = example5(x[i]);
    u2 = example5(Int(ceil(1.01* x[i])));
    global err[i] = norm(u1 - u2[1 : x[i]] , 2);
end

p1 = plot(x, log10.(err), color=:black, linewidth=2, xlabel=L"n", ylabel="Cauchy error", label=:none, alpha=1, grid=false)
# open("Fig1d.txt", "w") do file
    # for i in 1:length(x)
