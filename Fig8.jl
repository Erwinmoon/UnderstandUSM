...
p1 = plot(x, log10.(err1), color=:black, shape=:circle, linewidth=2, xlabel=L"n", label=:"1e-2", alpha=1, grid=false)
...
p2 = plot!(x, log10.(err2), color=:black, shape=:star5, linewidth=2, xlabel=L"n", label=:"1e-5", alpha=1, grid=false)


open("Fig8.txt", "w") do file
    for i in 1:length(x)
        write(file, "$(err1[i])\t$(err2[i])\t$(x[i])\n")
    end
end