using ApproxFun
using ApproxFunBase
using LinearAlgebra
using GenericLinearAlgebra
using SpecialFunctions
using Plots
using LaTeXStrings

function example6(n)
    ## test for Airy equation
    ## eps * u''(x) - x * u(x) = 0
    ## u(-1) = Ai(-sqrt3(1 / eps))
    ## u(1) = Ai(sqrt3(1 / eps))
    ## u_exact = Ai(sqrt3(1 / eps) * x);
    ## solution

    x = Fun();
    eps = 1e-2;

    ## innital date u0bound
    u0 = zeros(n , 1);

    ## diff mat D
    D = zeros(n , n);
    D[1:n, 1:n] = Derivative(Chebyshev() , 2)[1:n, 1:n];

    ## convert matrix S
    S = zeros(n , n);
    S = Conversion(Chebyshev(),Ultraspherical(2))[1:n , 1:n];

    ## multimat M
    M = zeros(n , n);
    M = Multiplication(x, Chebyshev())[1:n , 1:n];

    ## boundary conditions B
    B = zeros(2 , n);
    B = Dirichlet(-1..1)[1 : 2 , 1 : n];

    ## diff operator
    L = zeros(n , n);
    L = eps .* D  - S * M;
    L[3 : end , :] = L[1 : end - 2 , :];
    L[1 : 2 , :] = B;

    ## right hand side
    f = zeros(n , 1);
    u0 = S * u0;
    f[3 : end] = u0[1 : end - 2];
    f[1] = airyai(-1 / (eps)^(1/3));
    f[2] = airyai(1 / eps^(1/3));
    
    ## solve 
    # unum = L \ f;

    # Q, R = givensQR(L, 5);
    # Q, R = householderQR(L, 5);
    # unum = back_substitution(R, Q' * f)

    Q, R = householderQR([L f], 5);
    unum = back_substitution(R[: , 1 : end - 1], R[: , end])

    # unum = R \ (Q \ f);

    # F = qr(L);
    # unum = F.R\(F.Q \ f);
    return unum
end

function example8(n)
    ## test for Airy equation
    ## eps * u''(x) - x * u(x) = 0
    ## u(-1) = Ai(-sqrt3(1 / eps))
    ## u(1) = Ai(sqrt3(1 / eps))
    ## u_exact = Ai(sqrt3(1 / eps) * x);
    ## solution

    x = Fun();
    eps = 1e-2;

    ## innital date u0bound
    u0 = zeros(n , 1);

    ## diff mat D
    D = zeros(n , n);
    D[1:n, 1:n] = Derivative(Chebyshev() , 2)[1:n, 1:n];

    ## convert matrix S
    S = zeros(n , n);
    S = Conversion(Chebyshev(),Ultraspherical(2))[1:n , 1:n];

    ## multimat M
    M = zeros(n , n);
    M = Multiplication(x, Chebyshev())[1:n , 1:n];

    ## boundary conditions B
    B = zeros(2 , n);
    B = Dirichlet(-1..1)[1 : 2 , 1 : n];

    ## diff operator
    L = zeros(n , n);
    L = eps .* D  - S * M;
    L[3 : end , :] = L[1 : end - 2 , :];
    L[1 : 2 , :] = B;

    ## right hand side
    f = zeros(n , 1);
    u0 = S * u0;
    f[3 : end] = u0[1 : end - 2];
    f[1] = airyai(-1 / (eps)^(1/3));
    f[2] = airyai(1 / eps^(1/3));
    
    ## solve 
    # Q, R = givensQR(L, 5);
    # Q, R = householderQR(L, 5);
    # unum = R \ (Q \ f);

    Q, R = householderQR([L f], 5);
    unum = back_substitution(R[: , 1 : end - 1], R[: , end])

    # unum = L \ f;

    uexact = coefficients(Fun(airyai(1 / eps^(1/3) * x) , Chebyshev() , n));
    err = norm(unum - uexact , 2);  ## solution(Float64) - solution(exact)
    # F = qr(L);
    # unum = F.R\(F.Q \ f);
    # unum = F \ f;

    # err
    return err
end

function back_substitution(U, b)
    n = size(U, 1)
    x = zeros(n)
    for i = n:-1:1
        if U[i, i] == 0
            error("矩阵是奇异的，不能求解")
        end
        x[i] = b[i]
        for j = i+1:n
            x[i] -= U[i, j] * x[j]
        end
        x[i] /= U[i, i]
    end
    return x
end

function householderQR(A, k)
    m, n = size(A)
    Q = Matrix{Float64}(I, m, m)  # 初始化正交矩阵 Q
    R = copy(A)  # 初始化 R 为 A 的副本，将在算法中进行修改

    for i = 1:m
        # 提取第 i 列以下的部分，包括第 k 个对角元素以下的 k 个元素
        x = R[i:min(m,i+k), i]

        # 创建第 i 个 Householder 向量
        e1 = zeros(length(x))
        e1[1] = 1
        v = sign(x[1]) * norm(x) * e1 + x
        v /= norm(v)  # 归一化

        # 应用 Householder 变换到剩余的矩阵
        R[i:min(m,i+k), i:n] -= 2 * v * (v' * R[i:min(m,i+k), i:n])

        # 累积 Q 矩阵的变换
        Q[:, i:min(m,i+k)] -= 2 * (Q[:, i:min(m,i+k)] * v) * v'
    end

    # 由于数值精度问题，对 R 矩阵的对角线以下的元素进行清零处理
    R = triu(R)
    return Q, R
end




function givensQR(A, k)
    m, n = size(A)
    Q = Matrix{Float64}(I, m, m)

    for i = 1:n-1
        for j = i+1:min(m, i+k)
            x = A[i:min(m, i+k), i]
            rt = givens(x, j-i+1)
            Q[:, i:min(m, i+k)] = Q[:, i:min(m, i+k)] * rt'
            A[i:min(m, i+k), :] = rt * A[i:min(m, i+k), :]
            # println("j: ", j)
        end
    end

    return Q, A
end

function givens(x, j)
    xi = x[1]
    xj = x[j]
    r = sqrt(xi^2 + xj^2)
    cost = xi / r
    sint = xj / r
    R = Matrix{Float64}(I, length(x), length(x))
    R[1, 1] = cost
    R[1, j] = sint
    R[j, 1] = -sint
    R[j, j] = cost
    return R
end

# Plots
x = 10 : 10 : 200;
lx = size(x);
global errcauchy = zeros(lx[1] , 1);
global err = zeros(lx[1] , 1);
for i = 1 : 1 : lx[1]
    u1 = example6(x[i]);
    u2 = example6(Int(ceil(1.01* x[i])));
    global errcauchy[i] = norm(u1 - u2[1 : x[i]] , 2);
    global err[i] = example8(x[i]);
end

p1 = plot(x, log10.(errcauchy), color=:black, shape=:circle, linewidth=2, xlabel=L"n", label=:"Cauchy error", alpha=1, grid=false)
# p2 = plot!(x, log10.(ucoe[10 : 10 : 500]), color=:black, shape=:cross, linewidth=2, label=:"coefficients of airy function", alpha=1, grid=false)
p3 = plot!(x, log10.(err), color=:black, shape=:star4, linewidth=2, xlabel=L"n", label=:"Float64-uexact", alpha=1, grid=false)

# open("Fig5b.txt", "w") do file
    # for i in 1:length(x)
        # write(file, "$(errcauchy[i])\t$(err[i])\t$(x[i])\n")
    # end
# end